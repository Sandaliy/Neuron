/**
 * English is the presentation catalogue. Retained Russian entries fall back here for new keys.
 */
export const en = {
  'today.updatingPlan': 'Updating plan…',
  'study.extraLetter': '1 extra letter',
  'study.missingLetter': '1 missing letter',
  'study.changedLetter': '1 different letter',
  'study.answerAccepted': 'Correct',
  'study.answerIncorrect': 'Incorrect',
  'study.answerChanges': 'Spelling differences: {count}',
  'study.spelling.correct': 'Matching: {text}.',
  'study.spelling.extra': 'Extra: {text}.',
  'study.spelling.missing': 'Missing: {text}.',
  'study.spelling.incorrect': 'Replace {text} with {expected}.',
  'study.skills': 'Study skills',
  'study.skillsHint':
    'Enable skills for existing and future notes. Each has its own schedule. New cards join gradually within your study budget. Enabled cards keep their history; choose skills for each session in Today.',
  'study.yourAnswer': 'Your answer',
  'practice.response': 'Response mode',
  'practice.modeHint': 'Practice freely. Your study schedule and review history stay unchanged.',
  'practice.mode.reveal': 'Self-check',
  'practice.mode.typing': 'Typing',
  'practice.mode.listening': 'Listening',
  'practice.typingHint':
    'Choose one short text field for the answer. Long text, combined fields and yes/no fields are excluded.',
  'practice.listeningHint':
    'Choose Term alone on the front. Play the word, then reveal your chosen answer fields. Notes without a term are excluded.',
  'note.readyCards': '{count} cards ready now',
  'note.restartHint':
    'Start these cards from new again. Review history is preserved. To resume without resetting, use Return to study.',
  'library.noteCount': '{count} notes',
  'note.studyStatus': 'Study status',
  'study.moreDirections': 'Study directions',
  'study.directionHint':
    'Enable an independently scheduled direction for this note. Existing progress stays unchanged.',
  'study.acceptedAnswers': 'Accepted production answers, one per line',

  'practice.exit': 'Exit Practice',
  'time.today': 'Today',
  'time.tomorrow': 'Tomorrow',
  'time.relative': 'in {hours}h {minutes}m',
  'practice.resume': 'Continue practice',
  'practice.knownCount': '{known} / {total} known',
  'note.ready': 'Ready',
  'note.inReview': 'In review',
  'practice.completionCounts': '{known} known · {learning} still learning',
  'note.grammarLanguage': 'Choose the deck language to set up grammar for this note.',
  'note.grammarUnavailable':
    'No grammar fields are available for this language and part of speech.',
  'study.scope': 'Study decks',
  'study.scopeTemporary': 'For this session only. Your usual decks stay unchanged.',
  'study.scopeDefault': 'My study decks',
  'study.scopeAll': 'Select all decks for this session',
  'study.scopeNone': 'Clear',
  'study.scopeCount': '{count} decks',
  'study.noDecks': 'No decks selected',
  'study.chooseDecks': 'Choose for this session',
  'study.paused': 'Paused',
  'study.included': 'Include in Daily Study',
  'study.pause': 'Pause Daily Study',
  'study.participationHint': 'Pausing keeps progress and due dates. Practice stays available.',
  'study.stop': 'Stop',
  'study.typeAnswer': 'Type your answer',
  'study.checkAnswer': 'Check answer',
  'study.feedback.exact': 'Exact answer.',
  'study.feedback.correct': 'Correct answer.',
  'study.feedback.close': 'Close. Compare the spelling with the answer.',
  'study.feedback.incorrect': 'Compare your answer with the answer shown.',
  'study.chooseRating': 'Choose a rating yourself.',
  'study.listenPrompt': 'Listen and recall the word',
  'study.replay': 'Play / replay',
  'study.voiceUnavailable': 'A usable voice for this language is not available yet.',
  'study.listenFallback': 'You can always show the answer and continue.',
  'study.direction.recognition': 'Recognition',
  'study.direction.recall': 'Recall',
  'study.direction.production': 'Typing',
  'study.direction.cloze': 'Cloze',
  'study.direction.listening': 'Listening',
  'note.field.extra': 'Extra',
  'note.pos.preposition': 'Preposition',
  'note.field.pattern': 'Verb / complement pattern',
  'note.field.pastSimple': 'Past simple',
  'note.field.pastParticiple': 'Past participle',
  'note.field.countability': 'Countability',
  'note.countability.countable': 'Countable',
  'note.countability.uncountable': 'Uncountable',
  'note.countability.both': 'Countable and uncountable',
  'note.hint.pattern':
    'For example: auf + Akkusativ, jemandem etwas geben, or avoid doing. One pattern per line.',
  'note.grammarStored': 'Has details',
  'practice.chooseFields': 'Choose fields',
  'practice.chooseFieldsHint':
    'Each selected field must be populated. Changing fields starts a new run.',
  'practice.yes': 'Yes',
  'practice.no': 'No',
  'common.apply': 'Apply',
  'study.position': '{current} / {total}',
  'study.reviewsMetric': 'to review',
  'study.newMetric': 'new',

  'today.adjust': 'Adjust',
  'today.ready': 'ready',
  'today.nextReview': 'Next review',
  'today.noneScheduled': 'None scheduled',
  'today.practiceDeck': 'Choose a deck to practice',
  'practice.restart': 'Restart Practice',
  'practice.saved': 'Saved',
  'practice.saving': 'Saving…',
  'practice.changeFields': 'Change fields',
  'practice.changeWarning': 'Changing fields starts a new Practice run.',
  'learning.restart': 'Restart learning',
  'learning.restartTitle': 'Restart this deck?',
  'learning.restartBody':
    'Active eligible notes will start scheduled learning again. Review history is preserved. Already known notes stay known, and Practice is unaffected.',
  'learning.restarted': 'Learning restarted',

  'library.drag': 'Drag {name}',
  'library.dropInvalid': 'Cannot move here',
  'library.dropInside': 'Move inside',
  'library.dropHere': 'Insert here',
  'practice.title': 'Practice',
  'practice.scheduleSafe': 'A practice run only. Answers do not change your learning schedule.',
  'practice.front': 'Question field',
  'practice.back': 'Answer field',
  'practice.pool': '{count} / {total} notes',
  'practice.start': 'Start practice',
  'practice.progress': '{count} of {total} answered',
  'practice.learning': 'Still learning',
  'practice.know': 'Known',
  'practice.roundComplete': 'Round complete',
  'practice.cleared': 'Practice complete',
  'practice.remaining': '{count} notes still learning in this run.',
  'practice.repeat': 'Repeat still learning',
  'study.undo': 'Undo last answer',
  'study.plan': 'About {minutes} min · {reviews} reviews · {fresh} new',
  'study.allPlanned': 'All available work fits. Extra time will not add more cards.',
  'study.moreNew': 'Include more new material this time',
  'study.skill': 'Study direction',
  'study.mixed': 'Both ways',
  'study.recognition': 'Word → meaning',
  'study.recall': 'Meaning → word',
  'study.policyPause': 'New material is limited to leave room for reviews.',
  'study.summaryTime': '{minutes} min spent',
  'study.nextReturn': 'Next scheduled review: {time}',
  'study.waitForSave': 'Wait for answers to save, or retry the failed save.',
  'study.minutes': 'Time for this session',
  'study.defaultTime': 'Daily plan',
  'study.oneOff': 'Applies only to this session. Your daily plan stays the same.',
  'study.saveFailed': 'An answer could not be confirmed. Retry saving before continuing.',
  'study.answered': '{count} answered',
  'study.unavailable': 'Card unavailable',
  'study.unavailableBody': 'Return to Today and start a new session.',
  'study.again': 'Again',
  'study.hard': 'Hard',
  'study.good': 'Good',
  'study.easy': 'Easy',
  'study.intervalMinutes': '{count} min',
  'study.intervalDays': '{count} d',
  'study.reveal': 'Show answer',
  'study.complete': 'Session complete',
  'study.retryLater': 'The remaining cards are not due yet. Come back a little later.',
  'study.finish': 'Finish',
  'study.continue': 'Continue',
  'collection.changeDestination': 'Change destination',
  'import.noFile': 'No file selected',
  'auth.register.title': 'Create an account',
  'auth.register.submit': 'Create account',
  'auth.register.closed':
    'New accounts are closed right now. If you already have one, you can still sign in.',

  'auth.password.label': 'Password',
  'auth.password.tooShort': 'Use at least 10 characters.',
  'auth.password.tooLong': 'That is longer than 200 characters.',
  'auth.password.tooCommon':
    'That is one of the first passwords an attacker tries. Pick something else.',
  'auth.password.hint': 'At least 10 characters.',
  'auth.password.show': 'Show the password',
  'auth.password.hide': 'Hide the password',

  'auth.password.strength.fair':
    'Long enough. Another few characters, or one more word, is the easiest way to make it stronger.',
  'auth.password.strength.good': 'Good length. Another word would make it harder still.',
  'auth.password.strength.strong': 'Long enough that length is no longer the weak part.',
  'auth.password.confirmLabel': 'Type the password again',
  'auth.password.confirmMatch': 'Both fields match.',
  'auth.password.confirmMismatch': 'The two passwords are different.',

  'auth.signIn.title': 'Sign in',
  'auth.signIn.submit': 'Sign in',
  'auth.signIn.failed': 'That email and password do not go together.',
  'auth.signIn.forgot': 'Lost your password?',

  'auth.recoveryCodes.title': 'Your recovery codes',

  /**
   * The sentence the whole scheme rests on.
   *
   * There is no mail sender yet, so a recovery code is not a step towards
   * getting back in, it is the way in. Saying that plainly is the difference
   * between a person keeping the codes like a password and keeping them in a
   * note on their phone.
   */
  'auth.recoveryCodes.warning':
    'Anyone holding one of these codes can take over your account without your password. Keep them the way you would keep the password itself: written down somewhere private, or in a password manager. We cannot show them to you again.',
  'auth.recoveryCodes.subtitle':
    'Ten codes, each good once. The only way back in if you forget your password.',
  'auth.recoveryCodes.copy': 'Copy',
  'auth.recoveryCodes.copied': 'Copied',
  'auth.recoveryCodes.download': 'Download',
  'auth.recoveryCodes.confirm': 'I have saved them',
  'auth.recoveryCodes.remaining': 'Recovery codes left: {count}',
  'auth.recoveryCodes.low': 'Only {count} recovery codes left. Generate a new set.',
  'auth.recoveryCodes.none':
    'No recovery codes left. Generate a new set now, while you still know your password.',
  'auth.recoveryCodes.regenerate': 'Generate new codes',
  'auth.recoveryCodes.regenerateWarning':
    'This replaces every code you have. The old ones stop working immediately.',

  'auth.recovery.title': 'Sign in with a recovery code',
  'auth.recovery.hint': 'Type one of the codes you saved when you created the account.',
  'auth.recovery.invalid': 'That code is not right, or it has already been used.',
  'auth.recovery.exhausted':
    'Every recovery code for this account has been used. Get in touch so it can be reset by hand.',
  'auth.recovery.setPassword': 'Choose a new password',
  'auth.recovery.setPasswordHint':
    'The code has been used up. Choose a new password to finish signing in.',
  'auth.recovery.signedOutElsewhere': 'Everywhere else has been signed out.',

  'auth.twoFactor.title': 'Two-factor authentication',
  'auth.twoFactor.subtitle':
    'Optional. Adds a six digit code from an app on your phone to every sign in.',
  'auth.twoFactor.enable': 'Turn on 2FA',
  'auth.twoFactor.disable': 'Turn off 2FA',
  'auth.twoFactor.scan': 'Scan this with your authenticator app.',
  'auth.twoFactor.confirmHint': 'Type the code the app shows. Until you do, 2FA is not on.',
  'auth.twoFactor.codeLabel': 'Six digit code',
  'auth.twoFactor.invalid': 'That code is not right.',
  'auth.twoFactor.reused': 'That code has already been used. Wait for the app to show a new one.',
  'auth.twoFactor.unavailable': '2FA is not set up on this account.',
  /* Under the row in Settings, so the control says what it will do to a state. */
  'auth.twoFactor.on': 'On',
  'auth.twoFactor.off': 'Off',
  'auth.twoFactor.enabled': '2FA is on.',
  'auth.twoFactor.disabled': '2FA is off.',
  /** The one that matters. Losing a phone must not mean losing the account. */
  'auth.twoFactor.recoveryCodes.warning':
    'These ten codes are separate from your account recovery codes, and they exist for one reason: getting in when you no longer have your phone. Save them now. Without them, a lost phone means a lost account.',
  'auth.twoFactor.recoveryCodes.title': 'Codes for a lost phone',

  'auth.email.verifyTitle': 'Confirm your email',
  'auth.email.verifySent': 'If that address has an account, a link is on its way.',
  'auth.email.verifyRequired': 'Confirm your email address to start using the app.',
  'auth.email.resend': 'Send it again',
  'auth.email.verified': 'Email confirmed.',
  'auth.email.invalidToken': 'That link is not valid any more. Ask for a new one.',

  'auth.reset.title': 'Reset your password',
  'auth.reset.sent': 'If that address has an account, a link is on its way.',
  'auth.reset.done': 'Password changed. Everywhere else has been signed out.',

  'app.name': 'Neuron',
  'app.tagline': 'Spaced repetition that schedules your time, not your card count',

  'nav.today': 'Today',
  'nav.library': 'Library',
  'nav.settings': 'Settings',

  'common.cancel': 'Cancel',
  'common.back': 'Back',
  'common.continue': 'Continue',
  'common.save': 'Save',
  'common.close': 'Close',
  'common.retry': 'Try again',
  'common.loading': 'Loading',
  'common.signOut': 'Sign out',

  'auth.email.label': 'Email',
  'auth.signIn.noAccount': 'No account yet? Create one.',
  'auth.signIn.recover': 'Sign in with a recovery code',
  'auth.register.haveAccount': 'Already have an account? Sign in.',
  'auth.recoveryCodes.fileName': 'neuron-recovery-codes.txt',
  'auth.twoFactor.secretLabel': 'Cannot scan it? Type this into the app instead.',
  'auth.twoFactor.password': 'Your password',
  'auth.twoFactor.disableHint':
    'Turning it off needs your password and a code from the app, the same as turning it on did.',
  'auth.twoFactor.scanDone': 'I have added it',

  'auth.twoFactor.manualTitle': 'Enter the key by hand instead',
  'auth.twoFactor.manualHint':
    'Paste this key into your authenticator app under "add account by key". It is the same account the QR code sets up, so use one or the other, not both.',
  'auth.twoFactor.secretCopy': 'Copy the setup key',
  'auth.twoFactor.secretCopied': 'Setup key copied',
  'settings.changePasswordAction': 'Change your password',
  'settings.regenerateAction': 'Replace your recovery codes',
  'settings.deleteAccountAction': 'Delete account',

  'today.title': 'Today',
  'today.waitingIn': 'Waiting in',
  'today.waitingLabel': 'cards to review',
  'today.newLabel': 'new',
  'today.deckCounts': '{due} to review · {fresh} new',
  'today.waiting': 'Cards waiting: {count}',
  'today.newAvailable': 'New cards ready to start: {count}',
  'today.newAvailableHint':
    'How many of them a session actually introduces is decided when studying starts, from how much time the reviews already need.',
  'today.estimate': 'About {minutes} min',
  'today.estimateHint':
    'Worked out from a typical answer time. It turns into a real measurement once there are a few days of answers to measure.',
  'today.study': 'Study',
  'today.caughtUpTitle': 'You are caught up',
  'today.caughtUpBody':
    'Nothing is due right now. The next review will appear here when it is ready.',
  'today.emptyTitle': 'Nothing is waiting',
  'today.emptyBody': 'Cards appear here on the day they are due.',
  'import.replaceTitle': 'Replace the current input?',
  'import.replaceBody':
    'The example will replace everything in the input field. Cancel to keep your text.',
  'import.replaceConfirm': 'Replace with example',
  'library.newFolder': 'New folder',
  'library.folder': 'Folder',
  'library.deck': 'Deck',
  'library.folderSettings': 'Defaults for decks in this folder',
  'library.moveDeckInvalid': 'Choose a folder.',
  'deleted.originalLocation': 'Restores to this location',
  'deleted.liveContext': 'Original location',
  'deleted.permanent': 'Delete permanently',
  'deleted.permanentTitle': 'Delete “{name}” permanently?',
  'deleted.permanentBody': 'This content cannot be restored afterward. Review history will remain.',
  'deleted.permanentCheck': "I understand this can't be undone",
  'deleted.impact': 'Folders: {folders}. Decks: {decks}. Notes: {notes}. Cards: {cards}.',
  'deleted.permanentDone': 'Deleted permanently',
  'notes.actions': 'Actions for “{name}”',
  'import.requiredVocab': 'Word: term and translation.',
  'import.requiredBasic': 'Question: front (question) and back (answer).',
  'import.requiredCloze': 'Gap text: text with a hidden part.',
  'import.fieldNamesHelp': 'Use the field names below as table headings or JSON keys.',
  'import.extraFieldsHelp':
    'Add grammar and tags as needed. Put extra text in note; custom fields are not imported.',
  'import.grammarFields': 'JSON grammar fields',
  'import.grammarBoolean': 'Use true or false for separable, reflexive and uncountable.',
  'import.grammarValues':
    'gender: m/f/n; auxiliary: haben/sein; case: accusative/dative/genitive; countability: countable/uncountable/both; variant: BrE/AmE. pattern is text, one complement per line.',
  'import.partOfSpeechHelp':
    'partOfSpeech: noun, verb, adjective, adverb, preposition, phrase or other. Tables support article and plural; JSON supports all grammar fields.',

  'library.title': 'Library',
  'library.dueLabel': 'Cards waiting',
  'library.newLabel': 'Cards never answered',
  'library.expand': 'Show what is inside',
  'library.collapse': 'Hide what is inside',
  'library.emptyTitle': 'Your library is empty',
  'library.emptyBody': 'Create a deck for your notes. Use folders to organize decks.',
  'library.newDeck': 'New deck',
  'library.newDeckInside': 'New deck inside',
  'library.deckActions': 'Actions for {name}',
  'library.deckName': 'Name',
  'library.createTitle': 'New deck',
  'library.createInside': 'New deck inside {name}',
  'library.createSubmit': 'Create',
  'library.created': 'Created “{name}”',
  'library.rename': 'Rename',
  'library.renameTitle': 'Rename',
  'library.renameSubmit': 'Save the name',
  'library.move': 'Move',
  'library.moveTitle': 'Move {name}',
  'library.moveRoot': 'Top level',
  'library.moveSubmit': 'Move it here',
  'library.moved': 'Moved “{name}”',
  'library.moveSelf': 'This is the item being moved.',
  'library.moveDescendant': 'An item cannot move inside itself.',
  'library.moveSame': 'It is already here.',
  'library.moveUp': 'Move up',
  'library.moveDown': 'Move down',
  'library.settings': 'Settings',
  'library.settingsSubtitle':
    'Language and level. Unset values use defaults from the parent folder.',
  'library.targetLanguage': 'Language being learned',
  'library.nativeLanguage': 'Language of the explanations',
  'library.level': 'Level',
  'library.inherited': 'Inherited: {value}',
  'library.notSet': 'Not set',
  'library.delete': 'Delete',
  'library.deleteTitle': 'Delete {name}?',
  'library.deleteBody':
    'This item and its contents will leave the library and study sessions. You can restore them from Deleted.',
  'library.deleteSubmit': 'Delete',
  'library.deleted': 'Moved “{name}” to Deleted',
  'library.undo': 'Undo',
  'library.restored': '{name} is back',
  'library.openNotes': 'Open',
  'library.dragHint': 'Drag a deck onto another one to move it.',
  'deleted.title': 'Deleted',
  'deleted.segmentLabel': 'Deleted content',
  'deleted.decks': 'Folders & decks',
  'deleted.notes': 'Notes',
  'deleted.restore': 'Restore',
  'deleted.topLevel': 'Top level',
  'deleted.parentRequired': 'Restore the parent folder first.',
  'deleted.deckRequired': 'Restore the deck and its parent folders first.',
  'deleted.unknownDeck': 'Original deck unavailable',
  'deleted.decksEmptyTitle': 'No deleted folders or decks',
  'deleted.decksEmptyBody': 'Recoverable folders and decks appear here.',
  'deleted.notesEmptyTitle': 'No deleted notes',
  'deleted.notesEmptyBody': 'Deleted notes will appear here while they can still be restored.',
  'deleted.deckRestored': 'Restored “{name}”',
  'deleted.noteRestored': 'Note restored',
  'deleted.noteRestoredPartial': 'Note restored. {count} cards remain deleted.',
  'lang.de': 'German',
  'lang.en': 'English',
  'lang.ru': 'Russian',
  'lang.fr': 'French',
  'lang.es': 'Spanish',
  'lang.it': 'Italian',

  'note.new': 'New note',
  'note.edit': 'Note',
  'note.type': 'Type',
  'note.type.vocab': 'Word',
  'note.type.basic': 'Question',
  'note.type.cloze': 'Gap text',
  'note.deck': 'Deck',
  'note.tags': 'Tags',
  'note.tagsHint': 'Separated by commas.',
  'note.save': 'Save the note',
  'note.saving': 'Saving',
  'note.saved': 'Saved',
  'note.saveFailed': 'Not saved',
  'note.saveRetry': 'Save it again',
  'note.saveNeeded': 'Not saved yet',
  'note.preview': 'Cards this makes',
  'note.previewEmpty': 'Fill in the fields above and the cards appear here.',
  'note.previewFront': 'Front',
  'note.previewBack': 'Back',
  'note.willAdd': 'New',
  'note.willRemove': 'Goes',
  'note.reviewsOnCard': 'Answered {count} times',
  'note.typeChangeTitle': 'Change the type?',
  'note.typeChangeBody':
    'The existing cards will stop being studied. Their past reviews stay in history. New cards start with new schedules.',
  'note.typeChangeSubmit': 'Replace cards and change type',
  'note.conversionDraft':
    'Fill in the new fields, then apply. Until then, the saved note stays unchanged. New cards replace the old ones.',
  'note.conversionRequired': 'Complete required fields. Gap text needs at least one {{...}}.',
  'note.conversionApply': 'Apply type change',
  'note.conversionCancel': 'Cancel type change',
  'note.conversionWait': 'Save your current edits before changing the type.',
  'note.notFound': 'That note is not here. It may have been deleted.',
  'note.delete': 'Delete',
  'note.deleteTitle': 'Delete this note?',
  'note.deleteBody': 'The note and its cards move to Deleted. You can restore them there.',
  'note.deleted': 'Note moved to Deleted',
  'note.markKnown': 'Mark as known',
  'note.markActive': 'Return to study',
  'note.status.active': 'Studying',
  'note.status.known': 'Known',
  'note.status.suspended': 'Set aside',
  'note.status.draft': 'Draft',
  'note.missingDeck': 'Pick a deck for this note first.',
  'note.back': 'Back to the deck',
  'card.direction.recognition': 'Word to meaning',
  'card.direction.recall': 'Meaning to word',
  'card.direction.production': 'Say it yourself',
  'card.direction.cloze': 'Fill the gap',
  'card.direction.listening': 'Listen and say',

  'notes.title': 'Notes',
  'notes.allDecks': 'Every deck',
  'notes.count': 'Notes: {count}',
  'notes.countMore': 'Notes: {count} and more',
  'notes.search': 'Search',
  'notes.searchPlaceholder': 'Word, meaning or tag',
  'notes.filters': 'Filters',
  'notes.filterStatus': 'Study participation',
  'notes.filterTag': 'Tag or topic',
  'notes.filterSource': 'Import source name',
  'notes.filterCardState': 'Review stage',
  'notes.cardSummaryNone': 'No cards',
  'notes.cardSummaryMixed': 'Mixed · {count}',
  'notes.any': 'Any',
  'notes.sort': 'Order',
  'notes.sort.created': 'Oldest first',
  'notes.sort.alpha': 'Alphabetical',
  'notes.sort.rank': 'Most frequent words first',
  'notes.select': 'Select notes',
  'notes.selectDone': 'Exit selection',
  'notes.selected': 'Selected: {count}',
  'notes.selectAll': 'Select all shown',
  'notes.clearSelection': 'Clear the selection',
  'notes.bulkStatus': 'Already know these',
  'notes.bulkActive': 'Return to study',
  'notes.bulkMove': 'Move to a deck',
  'notes.bulkTags': 'Edit tags',
  'notes.bulkDelete': 'Delete',
  'notes.bulkDeleteTitle': 'Delete {count} notes?',
  'notes.bulkDeleteBody':
    'These notes and their cards will leave study sessions. You can restore them from Deleted.',
  'notes.bulkDeleted': 'Notes deleted: {count}',
  'notes.bulkDone': 'Notes changed: {count}',
  'notes.moved': 'Notes moved: {count}',
  'notes.deleted': 'Notes deleted: {count}',
  'notes.bulkTagsTitle': 'Tags for {count} notes',
  'notes.tagsAdd': 'Add tags',
  'notes.tagsRemove': 'Remove tags',
  'notes.tagsApply': 'Change the tags',
  'notes.bulkMoveTitle': 'Move {count} notes',
  'notes.newNote': 'New note',
  'notes.emptyTitle': 'Nothing in here yet',
  'notes.emptyBody': 'Write a note by hand, or paste a word list in and import it.',
  'notes.noMatchTitle': 'Nothing matches',
  'notes.noMatchBody': 'Try a shorter search, or clear the filters.',
  'notes.clearFilters': 'Clear the filters',
  'notes.cardCount': 'Cards: {count}',
  'notes.import': 'Import',
  'cardState.new': 'New',
  'cardState.learning': 'Learning',
  'cardState.review': 'In review',
  'cardState.relearning': 'Forgotten',

  'notes.studyFilterHint':
    'Study participation applies to the whole note. Review stage finds notes with at least one card at that stage.',
  'notes.organizeFilterHint':
    'Tags are your own labels, such as A1 or Travel. Source matches the exact source name saved during import. Tag choices come from the loaded notes; you can also enter a tag.',
  'notes.selectionHint': 'Tap notes to select them, then choose an action.',
  'notes.bulkRetryHint':
    'Some earlier groups may already be saved. You can retry this action safely.',
  'notes.updating': 'Updating results…',
  'import.mode': 'Import from',
  'import.mode.simple': 'List',
  'import.mode.table': 'Table',
  'import.mode.json': 'JSON',
  'import.mode.file': 'File',
  'import.help.simple':
    'Enter a word and its translation separately, then add it to the list. You can also paste a ready list below. Nothing is saved before you preview and import.',
  'import.help.table':
    'Copy cells from a spreadsheet, including headings if you have them. Word and translation go in separate columns. Check the column assignments in the preview. CSV is also accepted.',
  'import.help.json':
    'Paste an array of notes, or an object with noteType and notes. Use the example below for the selected note type.',
  'import.help.file':
    'Choose a text file exported from a spreadsheet or Anki, or a JSON file. You can inspect and edit its text below before importing.',
  'import.addRow': 'Add to the list',
  'import.appendError': 'Check the pasted list and its note type before adding another entry.',
  'import.exampleMeaning': 'to learn',
  'import.exampleHouse': 'house',
  'import.chooseFile': 'Choose file',
  'import.fileLabel': 'File',
  'import.fileSupport': 'TXT, CSV, TSV or JSON, up to 10 MB. Anki: export Notes in Plain Text.',
  'import.fileError': 'Could not read this file. Choose a TXT, CSV, TSV or JSON file up to 10 MB.',
  'import.previewHint': 'Preview checks each row before anything is written.',
  'import.example': 'Example and its cards',
  'import.useExample': 'Insert example',
  'import.generatedPreview': 'Cards from the first valid note',
  'import.supportedFields': 'Supported fields',
  'import.mediaLater': 'Text fields are supported. Images, audio and .apkg files are not imported.',
  'import.clozeHelp':
    'Hide a word with double braces: Ich {{lerne}} Deutsch. The card shows Ich [...] Deutsch. and reveals lerne. Anki syntax {{c1::lerne}} is also supported; matching numbers hide together.',
  'import.clozeError':
    'Add a hidden part in double braces, for example Ich {{lerne}} Deutsch. Close both braces.',
  'import.jsonError':
    'Cannot read row {row}. Use the JSON example: an array of notes, or an object with notes and noteType (vocab, basic or cloze). Check quotes, commas and brackets.',
  'import.tableError':
    'Cannot read row {row}. Check that its columns are assigned to supported fields.',

  'import.title': 'Import',
  'import.subtitle':
    'Paste a list or upload a file. Nothing is written until you have seen what it will make.',
  'import.deck': 'Import into',
  'import.paste': 'Paste the list here',
  'import.upload': 'Choose a file',
  'import.format': 'Format',
  'import.format.json': 'JSON list or object',
  'import.format.csv': 'CSV',
  'import.format.tsv': 'Spreadsheet text',
  'import.format.text': 'Plain text, one word per line',
  'import.format.anki': 'Anki export',
  'import.noteType': 'Note type',
  'import.read': 'Preview import',
  'import.columns': 'Which column is which',
  'import.columnIgnored': 'Ignore this column',
  'import.hasHeader': 'The first row is headings',
  'import.preview': 'What this will make',
  'import.previewCount': 'Rows read: {count}',
  'import.willCreate': 'Will be created: {count}',
  'import.willSkip': 'Will be skipped: {count}',
  'import.willMerge': 'Will be merged: {count}',
  'import.problemMissing': 'Missing: {fields}',
  'import.problemExample': 'The example does not contain the word',
  'import.problemDuplicate': 'Already in {deck}',
  'import.problemDuplicateFile': 'Twice in this list',
  'import.problemIssue': 'The generator flagged this row',
  'import.rowsWithProblems': 'Rows with something wrong: {count}',
  'import.checking': 'Checking for duplicates',
  'import.duplicates': 'Duplicates',
  'import.duplicatesFound': 'Already in the library: {count}',
  'import.duplicateAll': 'Default for duplicates',
  'import.rowAction': 'Action for row {row}',
  'import.useDefault': 'Use import default',
  'import.compatible': 'One matching note of the same type.',
  'import.ambiguous': 'Several matching notes of the same type. Merge is unavailable.',
  'import.incompatible': 'Matches have another note type. Merge is unavailable.',
  'import.effectiveAction': 'Will do: {action}.',
  'import.overridden': 'This row overrides the import default.',
  'import.undoBoundary':
    'Undo removes only notes and cards created by this import. Additions to existing notes stay.',
  'import.resolve.skip': 'Skip it',
  'import.resolve.merge': 'Fill empty fields',
  'import.resolve.create': 'Add it anyway',
  'import.start': 'Import {count} notes',
  'import.importing': 'Importing',
  'import.progress': '{done} of {total}',
  'import.merging': 'Merging into the notes you already have',
  'import.failed': 'The connection dropped after {done} of {total}. Nothing was written twice.',
  'import.resume': 'Carry on from where it stopped',
  'import.doneTitle': 'Import complete',
  'import.doneBody': 'Notes: {notes}. Cards: {cards}.',
  'import.openDeck': 'Open the deck',
  'import.undo': 'Take this import back',
  'import.undoTitle': 'Take back {count} notes?',
  'import.undoBody':
    'Notes and cards created by this import stop being studied. Additions to existing notes stay. Review history is kept.',
  'import.undoReviewed':
    'These cards have been answered {count} times between them. They stop being studied, but the answers remain in history.',
  'import.undoConfirm': 'I understand, take it back',
  'import.undone': 'Import undone',
  'import.triageLater': 'Sorting out which of these you already know arrives in phase 9.',
  'import.nothingToImport': 'Nothing here to import yet.',
  'import.tooManyProblems':
    'Rows missing a required field are not imported. Fix them in the file and read it again.',
  'import.copyPrompt': 'Copy the prompt',

  'prompt.variant': 'What kind of cards',
  'prompt.variant.vocabulary': 'Words',
  'prompt.variant.theory': 'Theory, as question and answer',
  'prompt.variant.cloze': 'Text with gaps',
  'prompt.about.vocabulary':
    'Words with translations, definitions, usage examples and relevant grammar.',
  'prompt.about.theory':
    'Questions and answers about definitions, formulas and dates. Each card tests one fact.',
  'prompt.about.cloze': 'Text with hidden words to recall in context.',
  'prompt.example': 'What a good answer looks like',
  'prompt.copied': 'Copied',
  'prompt.missing': 'Set the deck languages and level to fill the prompt in completely.',

  'settings.title': 'Settings',
  'settings.appearance': 'Appearance',
  'settings.theme': 'Theme',
  'settings.theme.system': 'System',
  'settings.theme.light': 'Light',
  'settings.theme.dark': 'Dark',
  'settings.language': 'Language',
  'settings.glass': 'Liquid glass',
  'settings.glass.off': 'Off',
  'settings.glass.subtle': 'Medium',
  'settings.glass.full': 'Max',
  'settings.glassScope': 'Where it applies',
  'settings.glassScope.floating': 'Panels only',
  'settings.glassScope.all': 'Panels and cards',
  'settings.glassScopeOff': 'Nothing to apply it to while the glass is off.',
  'settings.glassCapped.motion':
    'Your system asks for less movement, so the panels are running at Medium.',
  'settings.glassCapped.memory':
    'This device reports little memory, so the panels are running at Medium.',
  'settings.glassCapped.frames':
    'Scrolling dropped below 55 frames a second here, so the panels stepped down. Reload to try the full setting again.',
  'settings.motion': 'Animations',
  'settings.motion.system': 'Follow the system',
  'settings.motion.reduce': 'Off',
  'settings.security': 'Security',
  'settings.changePassword': 'Change password',
  'settings.currentPassword': 'Current password',
  'settings.newPassword': 'New password',
  'settings.passwordChanged': 'Password changed. Everywhere else has been signed out.',
  'settings.account': 'Account',
  'settings.deleteAccount': 'Delete account',
  'settings.deleteAccountWarning':
    'Deleting the account removes all decks, cards, review history and progress for good.',
  'settings.deleteAccountCodeHint':
    'Two-factor authentication is on, so a code from the app is needed too.',
  'settings.deleted': 'The account is closed.',

  /**
   * The fields of a note.
   *
   * Named for what a learner calls them, not for what the schema calls them.
   * The grammar labels stay in the language being learned, because that is what
   * a grammar book and a dictionary call them and translating them would mean
   * learning a second name for the same thing.
   */
  'note.field.term': 'Word',
  'note.field.reading': 'Reading',
  'note.field.translation': 'Translation',
  'note.field.definition': 'Definition',
  'note.field.example': 'Example',
  'note.field.exampleTranslation': 'Example, translated',
  'note.field.partOfSpeech': 'Part of speech',
  'note.field.mnemonic': 'Memory hook',
  'note.field.note': 'Note',
  'note.field.image': 'Image',
  'note.field.audio': 'Audio',
  'note.field.front': 'Front',
  'note.field.back': 'Back',
  'note.field.text': 'Text',
  'note.field.article': 'Article',
  'note.field.plural': 'Plural',
  'note.field.gender': 'Gender',
  'note.field.praeteritum': 'Präteritum',
  'note.field.partizip2': 'Partizip II',
  'note.field.auxiliary': 'Auxiliary',
  'note.field.separable': 'Separable',
  'note.field.case': 'Takes the case',
  'note.field.reflexive': 'Reflexive',
  'note.field.comparative': 'Comparative',
  'note.field.superlative': 'Superlative',
  'note.field.variant': 'Variant',
  'note.field.irregular': 'Irregular forms',
  'note.field.uncountable': 'Uncountable',

  'note.hint.reading': 'How it sounds, if that is not obvious from the spelling.',
  'note.hint.readingIpa': 'IPA, as /əˈbæn.dən/.',
  'note.hint.example': 'A sentence using the word. It has to contain the word itself.',
  'note.hint.mnemonic': 'Only if there is a real hook. A forced one is worse than none.',
  'note.hint.image': 'A web address. Uploading a file arrives later.',
  'note.hint.cloze': 'Wrap what should be hidden in double braces: Ich {{stehe}} früh auf.',
  'note.hint.case': 'Only when it is not the plain accusative.',
  'note.hint.comparison': 'Only when the comparison is irregular.',
  'note.hint.irregular':
    'Retained legacy forms. Use Past simple and Past participle for new entries; this value is never split automatically.',

  'note.section.grammar': 'Grammar',
  'note.section.extra': 'Extra',

  'note.pos.noun': 'Noun',
  'note.pos.verb': 'Verb',
  'note.pos.adjective': 'Adjective',
  'note.pos.adverb': 'Adverb',
  'note.pos.phrase': 'Phrase',
  'note.pos.other': 'Other',

  'note.gender.m': 'Masculine',
  'note.gender.f': 'Feminine',
  'note.gender.n': 'Neuter',

  'note.case.accusative': 'Accusative',
  'note.case.dative': 'Dative',
  'note.case.genitive': 'Genitive',

  'error.not_authenticated': 'Sign in to continue.',
  'error.not_allowed': 'That action was refused. Reload the page and try again.',
  'error.not_found': 'That is not here.',
  'error.invalid_request': 'Something in that request was not right.',
  'error.name_taken': 'That name is already used here.',
  'error.deck_cycle': 'A deck cannot be moved inside itself.',
  'error.restore_dependency': 'Restore the original parent deck or note first.',
  'error.unknown_note_type': 'That note type does not exist.',
  'error.invalid_note_fields': 'Those fields do not match the note type.',
  'error.rate_limited': 'Too many tries. Wait {seconds} seconds.',
  'error.registration_closed':
    'New accounts are closed right now. If you already have one, you can still sign in.',
  'error.weak_password': 'Pick a longer or less common password.',
  'error.email_taken': 'That address already has an account.',
  'error.invalid_credentials': 'That email and password do not go together.',
  'error.invalid_recovery_code': 'That code is not right, or it has already been used.',
  'error.no_recovery_codes': 'No recovery codes are left on this account.',
  'error.password_change_required': 'Choose a new password to finish signing in.',
  'error.email_not_verified': 'Confirm your email address first.',
  'error.two_factor_required': 'Type the code from your authenticator app.',
  'error.invalid_two_factor_code': 'That code is not right.',
  'error.two_factor_code_reused': 'That code has already been used. Wait for a new one.',
  'error.two_factor_unavailable': '2FA is not set up on this account.',
  'error.invalid_token': 'That link is not valid any more. Ask for a new one.',
  'error.direction_unavailable': 'That card direction is not available.',
  'error.cards_would_be_lost':
    'That change would delete cards you have already reviewed. Confirm it on the note first.',
  'error.sync_rejected': 'Those changes could not be saved. They are still on this device.',
  'error.service_unavailable': 'The server is not answering. Your work is saved on this device.',
  'error.network_unreachable':
    'This device cannot reach the server. Check the connection and try again. Your work is saved here.',
  'error.untrusted_origin':
    'The server does not recognise this web address. Open the app at its usual address and sign in there.',
  'error.unexpected': 'Something went wrong. Try again in a moment.',
  'error.internal_error':
    'Something went wrong at our end. Try again in a moment, and quote this if it keeps happening: {correlationId}',
} as const;

/** Every key the interface can ask for. */
export type MessageKey = keyof typeof en;

/** A catalogue in one language. Every key, no exceptions. */
export type Messages = Record<MessageKey, string>;
