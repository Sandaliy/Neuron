...
export const notes = pgTable(
  'notes',
  {
    ...
    /**
     * The note's own word, folded for comparing and for sorting.
     *
     * Generated by the database rather than written by the repository, so it
     * cannot be forgotten on one write path and cannot go stale after an edit.
     * `normaliseTerm` in packages/shared is the same expression in TypeScript,
     * and a test runs both over the same words.
     *
     * It exists because an import of five thousand rows has to ask "which of
     * these do I already have" without issuing five thousand queries. With this
     * indexed, the whole question is one `= any` per chunk.
     */
    termKey: text('term_key').generatedAlwaysAs(
      sql`left(lower(btrim(regexp_replace(coalesce(fields->>'term', fields->>'front', fields->>'text', ''), '[[:space:]]+', ' ', 'g'), ' ')), 200)`,
    ),
  },
  (table) => [
    ...
    /**
     * The duplicate check, and the alphabetical sort.
     *
     * Across the whole collection rather than one deck, because importing a
     * word that is already in another deck is exactly the duplicate worth
     * knowing about.
     */
    index('notes_user_term_key_idx')
      .on(table.userId, table.termKey)
      .where(sql`${table.deletedAt} is null`),
